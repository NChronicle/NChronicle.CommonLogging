...
namespace NPanda.NRegistry.Tests.Get.FromInterface {
    ...
    public class Reads : TestBase {
        ...
        private ITestInterface _object;

        [TestInitialize]
        public void Init () {
            this._object = this.Session.Get <ITestInterface>();
        }
        ...
        public void DefaultFromMethod() {
            Assert.AreEqual(default(string), this._object.MyCustomMethod());
        }

        [TestMethod]
        public void DefaultFromMethodWithParams() {
            Assert.AreEqual(default(string), this._object.MyCustomMethodWithParams("Hello World", 69));
        }

        [TestMethod]
        public void DefaultFromGenericMethod() {
            Assert.AreEqual(default(string), this._object.MyCustomGenericMethod(new TestStruct()));
        }

        [TestMethod]
        public void DefaultReferenceFromGenericReturnMethod() {
            Assert.AreEqual(default(TestObject), this._object.MyCustomGenericReturnMethod(new TestObject()));
        }

        [TestMethod]
        public void DefaultValueFromGenericReturnMethod() {
            Assert.AreEqual(default(int), this._object.MyCustomGenericReturnMethod(69));
        }
